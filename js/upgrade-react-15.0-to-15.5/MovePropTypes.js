  ...
  return root
    .find(j.Program)
  	.forEach(path => {
		...
		const rootVariableDeclarations = j(path)
                    ...
                    key: {
                     type: "Identifier",
                      name: "propTypes"
                    	...
                    	const properties = []
                    j(xx).find(j.Property).forEach(xxx => {
    	...
    	console.log(`React.createClass index: ${reactCreateClassDeclarationIndex}`)
        ...
        currBody.splice(reactCreateClassDeclarationIndex + 1, 0, reactComponentAssignment);
